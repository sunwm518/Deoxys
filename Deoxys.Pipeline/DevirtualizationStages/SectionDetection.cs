...
namespace Deoxys.Pipeline.DevirtualizationStages
{
    public class SectionDetection : IDevirtualizationStage
    {
        ...
        {
            var peSections = context.PeFile.Sections.Where(q => q.Name != ".text" && q.Name != ".rsrc" && q.Name != ".reloc" && q.Name.Contains("0") || q.Name.Contains("1") || q.Name.Contains("2")).ToList();
            if (!peSections.Any())
            {
            ...
            if (peSections.Count == 3)
            {
                ...
                context.Logger.Success($"Located NashaSection {context.Nasha2.Name} With Offset {context.Nasha2.Offset}");
                return true;
            }
            context.Logger.Error("Could not locate any NashaVM sections!");
            Console.ReadLine();
